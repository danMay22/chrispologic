...
export default function CategoriesAndProducts() {
  ...
  const CATEGORIES = [
    ...
    { id: 'bags',        label: t('categories.bags'),       icon: <ShoppingBag className='size-4' /> },
    { id: 'sale',        label: t('categories.onSale'),     icon: <Tag className='size-4' /> },
  ];

  const SORT_OPTIONS = [
    { id: 'default',    label: t('filter.featured') },
    { id: 'price-asc',  label: t('filter.priceLow') },
    { id: 'price-desc', label: t('filter.priceHigh') },
      ...
      <section className='pb-6'>
        <div className='px-10'>
          <div className='flex flex-col sm:flex-row gap-3 items-stretch sm:items-center'>
            ...
            >
              <SlidersHorizontal className='size-4' />{t('filter.filters')}
            </button>
            {hasFilters && (
              <button onClick={clearAll} className='flex items-center gap-1 text-xs font-semibold uppercase tracking-widest text-muted-foreground hover:text-foreground transition-colors'>
                <X className='size-3' /> {t('filter.clearAll')}
              </button>
            )}
          </div>
            ...
            <div className='mt-4 border p-5 grid grid-cols-1 sm:grid-cols-2 gap-6'>
              ...
              <div>
                ...
                <div className='flex gap-3'>
                  {[
                    { label: t('categories.all'), val: 'all' },
                    { label: t('filter.onSale'),  val: 'sale' },
                  ].map((opt) => (
                    <button
                      key={opt.val}
                      ...
                      className={cn(
                        ...
                        active === opt.val ? 'bg-foreground text-background border-foreground' : 'hover:bg-muted',
                      )}
                    >
                      {opt.label}
                    </button>
